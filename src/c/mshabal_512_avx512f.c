...
static void mshabal512_compress(mshabal512_context *sc, const unsigned char *buf0,
...
void mshabal512_init(mshabal512_context *sc, unsigned out_size) {
    ...
    mshabal512_compress(sc, sc->buf0, sc->buf1, sc->buf2, sc->buf3, sc->buf4, sc->buf5,
...
void mshabal512(mshabal512_context *sc, const void *data0, const void *data1, const void *data2,
    ...
    num = len >> 6;
    if (num != 0) {
        mshabal512_compress(sc, (const unsigned char *)data0, (const unsigned char *)data1,
...
void mshabal512_close(mshabal512_context *sc, unsigned ub0, unsigned ub1, unsigned ub2,
                      ...
                      unsigned ub13, unsigned ub14, unsigned ub15, unsigned n, 
                      void *dst0, void *dst1, void *dst2, void *dst3, void *dst4, void *dst5, 
                      void *dst6, void *dst7, void *dst8, void *dst9, void *dst10, void *dst11, 
    ...
    if (dst0 != NULL) {
        ...
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 0];
    }
    if (dst1 != NULL) {
        u32 *out;
        ...
        out = (u32 *)dst1;
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 1];
    }
    if (dst2 != NULL) {
        u32 *out;
        ...
        out = (u32 *)dst2;
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 2];
    }
    if (dst3 != NULL) {
        u32 *out;
        ...
        out = (u32 *)dst3;
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 3];
    }
    if (dst4 != NULL) {
        u32 *out;
        ...
        out = (u32 *)dst4;
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 4];
    }
    if (dst5 != NULL) {
        u32 *out;
        ...
        out = (u32 *)dst5;
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 5];
    }
    if (dst6 != NULL) {
        u32 *out;
        ...
        out = (u32 *)dst6;
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 6];
    }
    if (dst7 != NULL) {
        u32 *out;
        ...
        out = (u32 *)dst7;
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 7];
    }
    if (dst8 != NULL) {
        u32 *out;
        ...
        out = (u32 *)dst8;
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 8];
    }
    if (dst9 != NULL) {
        u32 *out;
        ...
        out = (u32 *)dst9;
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 9];
    }
    if (dst10 != NULL) {
        u32 *out;
        ...
        out = (u32 *)dst10;
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 10];
    }
    if (dst11 != NULL) {
        u32 *out;
        ...
        out = (u32 *)dst11;
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 11];
    }
    if (dst12 != NULL) {
        u32 *out;
        ...
        out = (u32 *)dst12;
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 12];
    }
    if (dst13 != NULL) {
        u32 *out;
        ...
        out = (u32 *)dst13;
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 13];
    }
    if (dst14 != NULL) {
        u32 *out;
        ...
        out = (u32 *)dst14;
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 14];
    }
    if (dst15 != NULL) {
        u32 *out;
        ...
        out = (u32 *)dst15;
        for (z = 0; z < out_size_w32; z++)
            out[z] = sc->state[off + MSHABAL512_FACTOR * (z << 2) + 15];
    }
}
...
void mshabal512_openclose_fast(mshabal512_context_fast *sc, void *message, void *termination,
    ...
    // Initial Rounds
#define M(i) _mm512_loadu_si512((__m512i *)message + i)
